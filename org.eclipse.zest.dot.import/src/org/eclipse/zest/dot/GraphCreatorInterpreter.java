...
final class GraphCreatorInterpreter implements IGraphCreator {
    ...
    /** Graph layout attributes in the DOT input and their Zest layout algorithms. */
    private enum Layout {
        TREE(new TreeLayoutAlgorithm(STYLE)),
        GRID(new GridLayoutAlgorithm(STYLE)), 
        RADIAL(new RadialLayoutAlgorithm(STYLE)), 
        SPRING(new SpringLayoutAlgorithm(STYLE));
    ...
    private void createAttributes(final EObject eStatementObject) {
        ...
        if (type.equals("edge")) {
            globalEdgeStyle = DotAst.getAttributeValue(eStatementObject, "style");
            globalEdgeLabel = DotAst.getAttributeValue(eStatementObject, "label");
        } else if (type.equals("node")) {
            globalNodeLabel = DotAst.getAttributeValue(eStatementObject, "label");
        } else if (type.equals("graph")) {
            String graphLayout = DotAst.getAttributeValue(eStatementObject, "layout");
            if (graphLayout != null) {
                Layout layout = Layout.valueOf(Layout.class, graphLayout.toUpperCase());
            ...
            if (eRhsContentElement.eClass().getName().equals("node_id")) {
                ...
                if (sourceNodeId != null && targetNodeId != null) {
                    ...
                    if (styleValue != null) {
                        ...
                        Style v = Style.valueOf(Style.class, globalEdgeStyle.toUpperCase());
                        graphConnection.setLineStyle(v.style);
                    }
                }
            }
